...
import * as documentationAgent from '../services/documentationAgent.js';

const router = Router();

// Run Documentation Agent tasks (readme, api, docstrings, markdown, summarize_repo, guide)
router.post('/run', async (req, res) => {
  ...
  try {
    ...
    await documentationAgent.runDocumentationAgent(
      ...
      (err) => {
        console.error('[Doc Agent Router SSE Error]', err);
        res.write(`data: ${JSON.stringify({ error: err.message })}\n\n`);
        res.end();
      }
    );
  } catch (error) {
    console.error('[Doc Agent Router Error]', error);
